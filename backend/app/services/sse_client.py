...
class TimerStreamClient:
    """One reconnecting SSE connection. Does not own grant detection."""
    ...
    async def run_forever(self, stop: asyncio.Event, on_event: EventHandler) -> None:
        ...
        if not url:
            logger.warning("Timer stream URL empty; SSE not started")
            return

        backoff = 1.0
        logger.info("MeiaCoin SSE starting url=%s", url)
        while not stop.is_set():
            ...
            try:
                ...
                continue
        logger.info("MeiaCoin SSE stopped")
